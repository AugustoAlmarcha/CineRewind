...
export default function ViendoCard({ serie, onAvanzar, onDescartar, onAbrirDetalle, onVerInfoEpisodio }) {
  ...
  const rutaImagen = serie.foto_siguiente || serie.poster_path;
  ...
  return (
    <div 
      ...
      className="relative w-56 h-84 rounded-2xl overflow-hidden shadow-lg border border-neutral-300/40 dark:border-white/10 group flex-shrink-0 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl cursor-pointer bg-[#141418]"
    >
      {imagenUrl ? (
        <img 
          src={imagenUrl} 
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/40 to-transparent flex flex-col justify-between p-4 pointer-events-none">
        <div className="flex justify-between items-center pointer-events-auto">
          ...
          <div className="flex items-center gap-1.5">
            {/* Botón X-Ray / Detalle del Capítulo */}
            <button
              onClick={(e) => {
                e.stopPropagation();
                onVerInfoEpisodio(serie);
              }}
              className="w-7 h-7 rounded-full bg-black/60 hover:bg-white hover:text-black text-white flex items-center justify-center text-xs font-black transition cursor-pointer border border-white/20"
              title="Ver detalle del capítulo y actores (X-Ray)"
            >
              ℹ
            </button>
        ...
        <div className="space-y-2.5 pointer-events-auto">
          ...
          >
            <span>✓</span> Marcar E{proximoEpisodio} visto
          </button>
        </div>
      </div>
    </div>
